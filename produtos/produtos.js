...
function mostrarProdutos() {
  ...
  pagina.forEach((produto) => {
    ...
    // Verificar se o utilizador pode apagar produtos (apenas gerentes e admins)
    ...
    const botaoApagar = podeApagar
      ? `<button class="apagarProdBTN" onclick="apagarProduto(${produto.id})">Apagar</button>`
      : "";
  ...
  try {
    await comLoader(async () => {
      ...
      if (response.ok) {
        ...
        alert(
          "Erro: " + (errorData.message || "Erro ao apagar produto"),
          "erro",
        );
      }
    });
